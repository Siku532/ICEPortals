...
@Component({
  ...
  styleUrls: ["./mtd-oos-report.component.scss"],
})
export class MtdOosReportComponent implements OnInit {
  title = "";
  zonePlaceholder = "";
  regionPlaceholder = "";
  clusterPlaceHolder = "";
  ...
  constructor(
    ...
    if(this.router.url=="/dashboard/mtd-oos-mt"){
      this.title = 'MTD OOS Dashboard-MT';
    }
    else{
      this.title = 'MTD OOS Dashboard-GT';
    }
  ...
  getBrandSKUOOS() {
    if (this.endDate >= this.startDate) {
      ...
      const obj = {
        ...
        type:this.router.url == "/dashboard/mtd-oos-mt"? 1:2
      };

      const url = "mtd-oos-dashboard"; 
